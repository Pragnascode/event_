...
import { AlertTargetType, RoleType } from "../generated/prisma";

const createAlertSchema = z.object({
  alertType: z.enum(["EVERYONE", "ROLE"]),
  targetRole: z.enum(["DELEGATE", "VOLUNTEER", "ORGANIZER", "EVERYONE"]).optional(),
  ...
  async (req: AuthedRequest, res) => {
    ...
    const alert = await prisma.alert.create({
      data: {
        ...
        targetType: alertType as AlertTargetType,
        targetRole: alertType === "ROLE" ? (targetRole as RoleType) : null,
        message,
        isPanic: false,
      },
    });
  ...
  async (req: AuthedRequest, res) => {
    ...
    const alert = await prisma.alert.create({
      data: {
        roomId,
        createdByRole: req.auth.roleType,
        targetType: AlertTargetType.EVERYONE,
        targetRole: null,
        message,
        isPanic: true,
      },
...
alertsRouter.get(
  ...
  async (req: AuthedRequest, res) => {
    ...
    const where =
      req.auth.roleType === "ADMIN" || req.auth.roleType === "ORGANIZER"
        ? { roomId }
        : {
            roomId,
            OR: [
              { isPanic: true },
              { targetType: AlertTargetType.EVERYONE },
              { AND: [{ targetType: AlertTargetType.ROLE }, { targetRole: req.auth.roleType }] },
